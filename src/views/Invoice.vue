	...
	methods: {
		...
		close() {
			...
			setTimeout(() => {
				...
				this.editedInvoice = { ...this.defaultInvoice };
				this.editedIndex = -1;
			}, 300);
		},

		editUpdateInvoice({ ...obj }) {
			const { fn, text } = obj;
			const invoice = { ...this.editedInvoice, customerId: this.id };
			fn(invoice);
			this.snackbar = true;
			this.text = text;
		},
		...
		save() {
			if (this.editedIndex > -1) {
				// edited customer
				this.editUpdateInvoice({
					fn: this.editInvoice,
					text: 'Invoice Successfully Edited',
				});
			} else {
				// new invoice
				this.editUpdateInvoice({
